...
begin
  try
    ...
    if frmCadProduto.bGravadoSucesso then
      lblContAlteracoes.Caption := AdicionaContador(lblContAlteracoes.Caption);
    frmCadProduto.Free;
  end;
end;
...
var
  oProduto: TProduto;
begin
  if Application.MessageBox('Deseja mesmo excluir ?', 'Você tem certeza ?', MB_ICONQUESTION or MB_YESNO) = IDYES then begin
    ...
    try
      ...
      fdqPrincipal.Active := false;
      fdqPrincipal.Active	:= true;
      lblContExclusoes.Caption := AdicionaContador(lblContExclusoes.Caption);
    end;
...
begin
  try
    ...
    if frmCadProduto.id_Produto>0 then begin
      ...
      fdqPrincipal.Active	:= true;
      fdqPrincipal.RefreshRecord();
      lblContInclusoes.Caption := AdicionaContador(lblContInclusoes.Caption);
      frmCadProduto.Free;
    end;
  end;
end;
...
function TfrmListaProdutos.AdicionaContador(QtdeAtual: String): string;
begin
