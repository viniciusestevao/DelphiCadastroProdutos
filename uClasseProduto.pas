...
begin
  ...
  try
    ...
    fdqProduto.SQL.Add('insert into tab_Produto (Codigo,Nome,Descricao,NroLote,Qtde) values (''' + Codigo + ''', ''' + Nome + ''', ''' + Descricao + ''', ''' + NroLote + ''', ' + FloatToStr(Qtde)  + ')') ;
    fdqProduto.ExecSQL;
...
begin
  ...
  try
    DataModule1.fdConnection.Close;
    sSQL := 'update tab_Produto set Codigo=''' + Codigo + ''', Nome = ''' + Nome + ''', Descricao= ''' + Descricao + ''', NroLote = ''' + NroLote + ''', Qtde = ' + FloatToStr(Qtde) + ' where id_Produto = ' + IntToStr(ID_Produto);
    DataModule1.fdConnection.Open;
    DataModule1.fdConnection.ExecSQL(sSQL);
    DataModule1.fdConnection.Close;
  except
...
begin
  ...
  if ID_Produto>0 then begin
    try
      ...
      Codigo    := fdqProduto.FieldByName('Codigo').Value;
      Nome	    := fdqProduto.FieldByName('Nome').Value;
      Descricao := fdqProduto.FieldByName('Descricao').Value;
      NroLote   := fdqProduto.FieldByName('NroLote').Value;
      Qtde      := fdqProduto.FieldByName('Qtde').Value;
